...
{
  // you need to fix this.  Your code goes here.
  if(lo == hi) return -1;
  //printf("%d", x[lo]);
  if(x[lo] == key){
      printf("%d", lo);
    return lo;
      ...
      lo++;
      return linear_search(lo,hi, key);
  }
}
...
// then return the integer location, otherwise return -1
{
  ...
  else if(key < x[mid]) return binary_search(lo, hi/2, key);
  //else(key > x[mid]) return binary_search
  */
  return -1;
}
